...
async def google_auth(
    ...
    return await auth_controller.google_auth(
        request=request,
        db = db
    )
...
async def get_current_user_profile(
    ...
    Raises:
        ...
        HTTPException(500): Database error
    """
    return auth_controller.get_current_user_profile(
...
async def create_user_admin(
    ...
    #     role=request.role
    # )
    return await auth_controller.create_user_admin(
        request=request,
        current_user=current_user,
        db=db
    )
...
async def setup_system(
    ...
    #     role=request.role
    # )
    return await auth_controller.setup_system(
        request=request,
        db=db
    )
