...
2. Run a cosine similarity search in ChromaDB against all indexed CVs
   for this requisition, returning the top 20 most relevant candidates.
3. Populate state.job_description and state.top_candidates.

ChromaDB metadata contract (set during CV upload / vectorize_cv.py):
    {"requisition_id": int, "source": str}   # source = original CV filename

Candidates are identified by *source* (filename) throughout the pipeline.
Application DB records are created later in Node 4 (save_results).
"""
...
from ..state import BatchScreeningState, CandidateDoc
