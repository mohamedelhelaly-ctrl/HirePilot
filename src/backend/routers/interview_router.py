...
from models.schemas import TranscriptChunkCreate
from stores.llm.whisper_service import transcribe_chunk, clear_session_buffer
from stores.llm.llm_config import llm_generic
from stores.llm.follow_up_prompts import build_followup_prompt
from graphs.subgraphs.liveInterview.liveInterview_graph import live_interview_subgraph
from graphs.subgraphs.liveInterview.liveInterview_state import LiveInterviewState

from controllers import RequisitionController, ApplicationController, InterviewController
...
async def _load_session_context(
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            db_session = await interview_controller.get_interview_session_by_id(session_id, db)
            if not db_session:
                return f"InterviewSession {session_id} not found"

            # Load Application → Candidate + Requisition
            application = await application_controller.get_application(
                application_id, db, include_relations=True
            )
            if not application:
            ...
            await interview_controller.update_interview_session(
                session_id,
        ...
        while True:
            ...
            if msg_type == "audio_chunk":
                ...
                async def _save_chunk(t: str, seq: int):
                    try:
                        async with AsyncSessionLocal() as db:
                            await interview_controller.create_transcript_chunk(
                                db,
...
async def list_sessions(application_id: int):
    ...
    try:
        async with AsyncSessionLocal() as db:
            sessions = await interview_controller.get_interview_sessions_by_application(application_id, db)
            application = await application_controller.get_application(application_id, db, True)
            candidate_name = (
                application.candidate.full_name
...
async def create_session(req: CreateSessionRequest):
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            application = await application_controller.get_application(req.application_id, db, True)
            if not application:
                raise HTTPException(
            ...
            db_session = await interview_controller.create_interview_session(session_data, db)
